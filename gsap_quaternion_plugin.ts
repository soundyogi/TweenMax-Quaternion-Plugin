 ...
 */
	public class QuaternionsPlugin extends TweenPlugin {
		const API:Number = 2; //If the API/Framework for plugins changes in the future, this number helps determine compatibility
		
		const _RAD2DEG:Number = 180 / Math.PI; //precalculate for speed
		
		var _target:Object;
		var _quaternions:Array = [];
		
		
		function QuaternionsPlugin() {
			super("quaternions");
			_overwriteProps.pop();
		}
		
    function _onInitTween(target:Object, value:*, tween:TweenLite):Boolean {
			if (value == null) {
				return false;
			}
			_target = target;
		...
		function _initQuaternion(end:Object, p:String):void {
		...
		function _kill(lookup:Object):Boolean {
